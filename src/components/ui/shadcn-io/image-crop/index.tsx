...
const centerAspectCrop = (
    ...
    centerCrop(
        aspect
            ...
            : { x: 5, y: 5, width: 90, height: 70, unit: "%" },
        mediaWidth,
        mediaHeight
    );
...
const getCroppedImage = async (
    ...
    pixelCrop: PixelCrop,
    maxImageSize: number,
    originalFormat: string = "image/jpeg",
    quality: number = 0.95
): Promise<string> => {
    ...
    // Calculate the actual crop dimensions in the original image resolution
    const cropWidth = pixelCrop.width * scaleX;
    const cropHeight = pixelCrop.height * scaleY;
    const cropX = pixelCrop.x * scaleX;
    const cropY = pixelCrop.y * scaleY;

    // Set canvas to the actual crop size (preserving original resolution)
    canvas.width = Math.round(cropWidth * scaleFactor);
    canvas.height = Math.round(cropHeight * scaleFactor);

    console.log("Image cropping details:", {
        originalSize: `${imageSrc.naturalWidth}x${imageSrc.naturalHeight}`,
        displaySize: `${imageSrc.width}x${imageSrc.height}`,
        cropArea: `${pixelCrop.width}x${pixelCrop.height}`,
        actualCropSize: `${cropWidth}x${cropHeight}`,
    ...
    ctx.drawImage(imageSrc, cropX, cropY, cropWidth, cropHeight, 0, 0, canvas.width, canvas.height);

    // Use original format with high quality
    ...
    if (blob.size > maxImageSize) {
        ...
        const newQuality = originalFormat === "image/jpeg" ? Math.max(0.7, quality - 0.1) : quality;
        const newScaleFactor = newQuality === quality ? scaleFactor * 0.9 : scaleFactor;
